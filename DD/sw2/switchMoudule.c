...
// GPIO driver
//================================
//sudo tail -f /var/log/messges 로 커널log를 실시간으로 처리
...
struct cdev gpio_cdev; // 캐릭터 디바이스 드라이버 구조체
...
static struct hrtimer stop_watch;
//switch 입력이 두개가 들어온다.


enum hrtimer_restart my_stop_watch(struct hrtimer *timer)
{
...
static irqreturn_t isr_func(int irq, void *data)
{
   // hrTimer를 위한 변수
	...
	unsigned long stop_watch_time = 10000000L;//10ms unit:ns
	static int count=0;
...
//================================================================================================
	...
	//=========================================================================================
			stime = ktime_set(0, stop_watch_time);
			//hrtimer_init(타이머구조체 주소값, 등록할 타이머값,상대시간으로설정)
			hrtimer_init(&stop_watch, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
			stop_watch.function = &my_stop_watch;
			printk(KERN_INFO "startTime\n");
			hrtimer_start(&stop_watch, stime, HRTIMER_MODE_REL);
	...
	else if(irq == switch_irq2)
	{
		if (!flag2)
		{
			...
			//hrtimer_init(타이머구조체 주소값, 등록할 타이머값,상대시간으로설정)
			hrtimer_init(&hr_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
			hr_timer.function = &my_stop_watch;
			printk(KERN_INFO "startTime\n");
			hrtimer_start(&hr_timer, ktime, HRTIMER_MODE_REL);
...
static ssize_t gpio_write(struct file *fil, const char *buff, size_t len, loff_t *off)
{
	...
	if (endptr != NULL)
	{
		// pid값을 통해 task_struct구조체의 주소값을 확인  task의 주소값 =user 프로그램
...
static int __init initModule(void)
{
	...
	gpio_cdev.owner = THIS_MODULE; //소유권
	count = 1;
	...
	// 현재 GPIO_SW1핀이 사용중인지 확인하고 사용권한 획득
	...
	// 현재 GPIO_SW2핀이 사용중인지 확인하고 사용권한 획득
	...
	if (err)
	{
		printk(KERN_INFO "Error request_irq1\n");
		return -1;
	}
    
    //SW2 irq 설정
    switch_irq2= gpio_to_irq(GPIO_SW2);// 어떤 irq 가 왔는지 구분가능한 번호를 리턴
    
	err = request_irq(switch_irq2, isr_func, IRQF_TRIGGER_RISING, "switch2", NULL);
	if (err)
	{
		printk(KERN_INFO "Error request_irq2\n");
		return -1;
	}
