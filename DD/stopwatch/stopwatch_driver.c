...
enum hrtimer_restart myStopwatch_callback(struct hrtimer *timer)
{
	...
	for(i=0;i<8;i++)
    gpio_set_value(segs[i],seg_led[sec_7][i]);
...
static irqreturn_t isr_func(int irq, void *data)
{
	...
	unsigned long stopwatchTime = 	10000000L; //10ms unit:ns
...
static int __init initModule(void)
{
	...
	// 현재 SW_STOP이 사용중인지 확인하고 사용권한 획득
	err = gpio_request(SW_STOP, "STOP");
	if (err == -EBUSY)
	{
	...
	switch_irq1 = gpio_to_irq(SW_STOP);
	err = request_irq(switch_irq1, isr_func, IRQF_TRIGGER_RISING, "sw_STOP", NULL);
	if (err)
	{
	...
	// 현재 SW_PAUSE이 사용중인지 확인하고 사용권한 획득
	err = gpio_request(SW_PAUSE, "SW2");
	if (err == -EBUSY)
	{
	...
	switch_irq2 = gpio_to_irq(SW_PAUSE);
	err = request_irq(switch_irq2, isr_func, IRQF_TRIGGER_RISING, "sw_PAUSE", NULL);
	if (err)
	{
		printk(KERN_INFO "Error request_irq\n");
		return -1;
